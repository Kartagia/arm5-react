 ...
 * start of a xml name.
 */
export function validNameNonStartCodePoint(codePoint) {
  return (
    ["-", ".", ["0", "9"], "\u00b7", ["\u0300", "\u036f"], ["\u203f", "\u2040"]]
      .map((range) =>
        range instanceof Array
...
export function validNameCodePoint(codePoint) {
  return (
    validNameStartCodePoint(codePoint) || validNameNonStartCodePoint(codePoint)
  );
}
...
export function createQName(localName, { prefix = null, uri = undefined }) {
  ...
  if (!validKey(localName) || localName.indexOf(":") >= 0) {
    throw new SyntaxError(`Invalid local name`);
  }
  if (prefix && (!validKey(prefix) || prefix.indexOf(":") >= 0)) {
...
export function validCustomId(tested) {
  return validKey(tested) && !/^xml:/i.test(tested);
}
