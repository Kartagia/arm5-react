...
import { isGregorianLeapYear } from './modules.temporal.js';
...
export class seasonOfYear {
  ...
  static fromJulianDate(day, year, month = undefined) {
    ...
    while (dayOdYear < 0) {
      year--;
      amount = 365 + (isGregorianLeapYear(year) ? 1 : 0);
      dayOdYear += amount;
    }
    while (dayOfYear > (amount = 365 + (isGregorianLeapYear(year) ? 1 : 0))) {
      dayOfYear -= amount;;
      year++;
    }
  ...
  static fromDate(other) {
    ...
    if (isGregorianLeapYear(year) && dayOfYear > daysOfYears[1]) {
      // Add leap day to dates following it
      dayOfYear++;
    }
    if (isGregorianLeapYear(year) && dayOfYear > daysOfYear[1]) {
      // Remove leap day
      dayOfYear--;
    }
